...
import { SelectOptionsForm } from "./type";
import { selectOptions } from "./options";

type UseTextFieldFormReturn = Record<
  InputFormKey,
  GetInputs<MuiConfig, SelectOptionsForm>[]
>;

const useSelectForm = (): UseTextFieldFormReturn => {
  const fieldInputs = useFieldOptions<SelectOptionsForm>();
  const gridItemInputs = useGridItemOptions<SelectOptionsForm>({
    depPrefix: "gridProps",
    responsivePath: (name) => {
  ...
  return {
    fieldInputs,
    gridItemInputs,
    propsInputs: selectOptions,
  };
};

export { useSelectForm };
